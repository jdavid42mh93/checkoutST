...
url = 'https://ema.getduna.com/api/v1/merchants/'+str(merchant_id)+'/configurations'
headers = {'Authorization': 'Bearer ' + token}

r = requests.get(url, headers=headers)
data = r.json()

with open('configuraciones.json', 'w') as json_file:
    json.dump(data, json_file, indent=4, sort_keys=True)
print(r.status_code)

with open('configuraciones.json') as file:
    data = json.load(file)

    for configuracion in data['configuration']:
        print('id:', configuracion['id'])
        print('')