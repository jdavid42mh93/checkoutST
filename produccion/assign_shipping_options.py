...
payload = {
	"shipping_options": ["delivery","pickup","dine_in"]
}
with open('locales.json') as file:
    stores = json.load(file)

    for local in stores['stores']:
        url= 'https://payment.getduna.com:443/api/v1/merchants/'+merchant_id+'/stores/'+local["name"]+'/processors/assign-available-for?method_type=pos'
